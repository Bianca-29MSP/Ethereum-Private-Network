...
// Configurações
const minerName = 'miner1';
const minerUrl = 'http://localhost:8545';
...
// Contas gerenciadas por este minerador (com saldos observados no relatório)
const accountsMiner1 = [
...
async function refillAllAccounts() {
  try {
    ...
    const accountsEstimate = accountsMiner1.length - 1; // Menos a própria master
    ...
    console.log(`🔍 ${accountsMiner1.length} contas configuradas para este minerador`);
    
    // Filtra a lista para remover a master account
    const accountsToRefill = accountsMiner1.filter(acc => 
