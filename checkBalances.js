...
// Função utilitária para converter Wei para ETH
function fromWei(wei) {
  return Web3.utils.fromWei(wei, 'ether');
}

(async () => {
  console.log("===== Verificação de saldos e gas price =====");

  for (let i = 0; i < minerUrls.length; i++) {
    const url = minerUrls[i];
    ...
    try {
      ...
      console.log(`Gas Price atual: ${fromWei(gasPrice)} ETH`);

      for (let j = 0; j < accounts.length; j++) {
        const address = accounts[j];
        try {
          const balanceWei = await web3.eth.getBalance(address);
          const balanceEth = fromWei(balanceWei);
          console.log(`Conta ${j + 1}: ${address} → Saldo: ${balanceEth} ETH`);
        } catch (err) {
          console.error(`Erro ao buscar saldo da conta ${address}:`, err.message);
        }
      }

    } catch (err) {
      console.error(`Erro ao conectar ao minerador ${url}:`, err.message);
    }
  }
})();
